    ...
    ui(new Ui::DisplaySysInfo)
{
    ...
    timer->start(1000);//단위:milliseconds::10^(-3)s

    getNode = new GetNode();//getnode 생성자의 동적할당?+getnod생성자.실행.
    ...
    a15Volt = a15Ampere = a15Watt = "";//Qstring을초기화?, UI에서해서그런가?
    a7Volt = a7Ampere = a7Watt = "";
    gpuVolt = gpuAmpere = gpuWatt = "";
    memVolt = memAmpere = memWatt = "";

    ARMSensorCurve = new QwtPlotCurve();//graph
...
DisplaySysInfo::~DisplaySysInfo()//소멸자.
{
    getNode->CloseINA231();
    delete ui;
}
...
{
    ui->qwtPlotSensor->setAxisScale(QwtPlot::yLeft, 0, 5);//watt graph의. x축,y축의. 수치and name.
    ...
    ui->qwtPlotSensor->setCanvasBackground(QBrush(QColor(0, 0, 0)));//Change the background of the plotting area.
    ui->qwtPlotSensor->setTitle("Watt Graph");

    ARMSensorCurve->attach(ui->qwtPlotSensor);//A15
    ARMSensorCurve->setPen(QColor(50, 160, 140));

    MEMSensorCurve->attach(ui->qwtPlotSensor);//mem
    MEMSensorCurve->setPen(QColor(255, 0, 0));

    KFCSensorCurve->attach(ui->qwtPlotSensor);//A7
    KFCSensorCurve->setPen(QColor(0, 255, 0));

    G3DSensorCurve->attach(ui->qwtPlotSensor);//Gpu
...
void DisplaySysInfo::DisplaySensor()
{
    ...
    ui->ARMuVlcd->display(a15Volt);//화면 표시
    ui->ARMuAlcd->display(a15Ampere);
    ui->ARMuWlcd->display(a15Watt);

    ui->KFCuVlcd->setPalette(QColor(100, 100, 100));//색칠
...
{
    a15Volt.sprintf("%.3f", getNode->armuV);//문자열,복사.
...
void DisplaySysInfo::displayCpuFrequency()
{
    ...
    ui->CPU1FreqEdit->setText(getNode->GetCPUCurFreq(1));//cpu저장.파일로부터.읽어온.데이터값.
